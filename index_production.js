...
app.use(session({
    ...
    cookie: { secure:true, httpOnly:false,sameSite:'None',maxAge: 1000*60*10 }
}));
let allowedDomains = []
let origine = 'null'
...
app.post('/init.php', (req, res) => {
    /*let origin = req.get('origin')
    console.log('origin: '+origin)
    cors1.origin = origin
    cors1.credentials = true
    res.send('done')*/
})
...
let privateKey = fs.readFileSync( '/etc/letsencrypt/live/ydelocation.com/privkey.pem' )
let certificate = fs.readFileSync( '/etc/letsencrypt/live/ydelocation.com/fullchain.pem' )
